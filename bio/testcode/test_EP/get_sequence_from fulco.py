...
# GENE_short_name --> ENSG name
def get_ensembl_id(gene_short_name):
    url = f'https://rest.ensembl.org/xrefs/symbol/homo_sapiens/{gene_short_name}?content-type=application/json'
    response = requests.get(url)
    if response.ok:
        data = response.json()
        ensembl_ids = [item['id'] for item in data if item['id'].startswith('ENSG')]
        return ensembl_ids[0] if ensembl_ids else None
    else:
        return None
